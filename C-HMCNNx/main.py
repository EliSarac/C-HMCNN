...
def create_sub_samples(train_loader, sample_size):
    sub_samples = random.sample(list(train_loader.dataset), sample_size)
    return torch.utils.data.DataLoader(dataset=sub_samples, batch_size=train_loader.batch_size, shuffle=True)

def augment_data(train_dataset, noise_factor=0.1):
    augmented_data = []
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        # Create sub-samples
        sub_train_loader = create_sub_samples(train_loader, sample_size=len(train_loader.dataset) // 4)
        
        # Print the number of samples used for training in this epoch
        print(f'Epoch {epoch + 1}: Number of samples used for training: {len(sub_train_loader.dataset)}')

        for i, (x, labels) in enumerate(sub_train_loader):
            x = x.to(device)
